...
void If_Node::update_cache(){
	if(inodes[1]->cache->type_id == bool_t_id && ((Bool_t*)inodes[1]->cache)->value){
...
void If_Node::prepare_program(std::vector<Node*>& program){
	inodes[1]->prepare_program(program);
	program.push_back(this);

	then_program.clear();
	inodes[0]->prepare_program(then_program);
