...
int Graphics_View::handle(int e){
	...
	case FL_PUSH:
		...
		if(hover) hover->mouse_press_event(Fl::event_x(),Fl::event_y());
		return 1;
	case FL_RELEASE:
		if(click){
			mouse_click_event(Fl::event_x(), Fl::event_y(), Fl::event_button());
			if(hover) hover->mouse_click_event(Fl::event_x(),Fl::event_y(),Fl::event_button());
			click= false;
		}
	...
	case FL_MOUSEWHEEL:
		...
		((Graphics_View*)Fl::belowmouse())->mouse_wheel_event(Fl::event_dx(),Fl::event_dy());
		return 1;
	case FL_DND_ENTER:
...
void Graphics_View::mouse_move_event(int,int){}
void Graphics_View::mouse_click_event(int, int, int){}
void Graphics_View::mouse_drag_event(int, int, int){}
void Graphics_View::mouse_press_event(int, int, int){}
